...
const OpenAIApiSettings: React.FC = () => {
  ...
  useEffect(() => {
    const loadApiKey = async () => {
      ...
      try {
        ...
        const key = await Promise.race([getApiKey('openai'), timeout]);
  ...
  const handleSave = async () => {
    ...
    try {
      // Race the API call against the timeout
      await Promise.race([saveApiKey('openai', apiKey), timeout]);
      ...
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Enter your OpenAI API key to track usage and costs. Your key is stored locally and never
            sent to our servers.
          </Typography>
        </Grid>
        ...
        {isValid === false && (
          <Grid item xs={12}>
            <Alert severity="error">
              Invalid API key. OpenAI API keys typically start with "sk-" and are at least 30
              characters long.
            </Alert>
          </Grid>
        )}
        ...
        <Grid item xs={12} sx={{ mt: 2 }}>
          ...
          >
            ...
            <Typography variant="body2" paragraph>
              OpenAI API usage is billed based on the number of tokens processed. Current pricing
              (as of 2025):
            </Typography>
            <Box
              component="table"
            ...
            <Typography variant="body2" sx={{ mt: 2 }}>
              Check the{' '}
              <Link href="https://openai.com/pricing" target="_blank" rel="noopener noreferrer">
                OpenAI pricing page
              </Link>{' '}
