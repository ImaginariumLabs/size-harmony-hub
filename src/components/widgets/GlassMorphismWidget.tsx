...
const GlassMorphismWidget: React.FC<GlassMorphismWidgetProps> = ({
  ...
  const [size, setSize] = useState<Size>(storedSettings.size || initialSize);
  const [theme, setTheme] = useState<Theme>(storedSettings.theme || initialTheme);
  ...
  // Fetch data from real API services with timeout protection
