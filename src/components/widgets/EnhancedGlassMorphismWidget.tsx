...
const EnhancedGlassMorphismWidget: React.FC<EnhancedGlassMorphismWidgetProps> = ({
  ...
  // Add debugging
  console.log('EnhancedGlassMorphismWidget rendering with props:', {
    widgetId,
    providerId,
    initialSize,
    initialPosition,
    hasOpenSettings: !!onOpenSettings,
    hasOnClose: !!onClose,
    isElectronApp
  });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (isElectronApp) {
        try {
          // Use a timeout for the fallback as well
          const data = await Promise.race([
            getApiCost(providerId),
          ...
          // Use mock data as a last resort to prevent UI from breaking
          const mockData = {
            total: 0,
            change: 0,
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      if (isDragging) {
        ...
        const newWidth = Math.max(120, initialDimensions.width + deltaX);
        const newHeight = Math.max(60, initialDimensions.height + deltaY);
  ...
  }, [isDragging, dragOffset, isResizing, ensureWithinBoundaries, position, size, widgetId, updateWidget, customSize, initialDimensions.width, initialDimensions.height, resizeStartPos.x, resizeStartPos.y]);

  // Position widget within viewport boundaries
  useEffect(() => {
