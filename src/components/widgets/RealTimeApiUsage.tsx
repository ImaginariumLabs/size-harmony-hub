...
const RealTimeApiUsage: React.FC<RealTimeApiUsageProps> = ({
  ...
  // Function to fetch data with timeout protection
  const fetchData = async () => {
    ...
    try {
      ...
      if (provider) {
        // Fetch data for a specific provider with timeout
        const result = await Promise.race([
          fetchApiData(provider),
          timeout
        ]);
        setData(result);
      } else {
        // Fetch data for all providers with timeout
        const result = await Promise.race([
          fetchAllApiData(),
          timeout
        ]);
      ...
      // Create fallback data to prevent UI from breaking
      if (!data) {
        if (provider) {
          // Fallback for a specific provider
          setData({
            total: 0,
            change: 0,
  ...
  }, [refreshInterval, provider, fetchData]);
