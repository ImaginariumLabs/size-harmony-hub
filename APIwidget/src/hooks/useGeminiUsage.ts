...
import { validateApiKey, fetchUsageData, countTokens } from '../services/geminiService';
...
export function useGeminiUsage() {
  ...
  const fetchUsage = async () => {
    ...
    try {
      ...
      let storedData = localStorage.getItem('gemini_usage_data');
      ...
      const flashModel = usageData.models_used.find((m: any) => m.model === 'gemini-2.0-flash');
      if (flashModel) {
