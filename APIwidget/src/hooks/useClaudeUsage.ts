...
import { validateApiKey, fetchUsageData, countTokens } from '../services/claudeService';
...
export function useClaudeUsage() {
  ...
  const fetchUsage = async () => {
    ...
    try {
      ...
      let storedData = localStorage.getItem('claude_usage_data');
      ...
      const sonnetModel = usageData.models_used.find((m: any) => m.model === 'claude-3-5-sonnet');
      if (sonnetModel) {
      ...
      const todayCost = usageData.daily_costs.find((d: any) => d.date === today);
      if (todayCost) {
      ...
      usageData.daily_costs.sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
